...
class Model(nn.Module):
    def __init__(self,input_shape):
        ...
        # torch.nn.init.xavier_uniform(self.rnn.weight_hh_l0)
...
def rnn_model(lr,epochs,batch_size,X_train,y_train,X_test,y_test,validation,using_all,verbose,device):
  ...
  dataset_train=torch.utils.data.TensorDataset(X_t_train,y_t_train)
  ...
  epoch=0
  count=0
  loss_ori_each=np.zeros(n_epochs)
  while epoch <n_epochs:
      ...
      for i, data in enumerate(train_loader, 0):
          ...
          n += 1
      model.eval()
      y_a,x_save = model(X_t_train)
      accuracy=np.ones(y_a.shape[1])
      for h in range(y_a.shape[1]):
        accuracy[h]=get_accuracy(y_t_train,y_a[:,h,0])
      if validation==True:
        y_v,x_save_val = model(X_t_val)
        val_accuracy=np.ones(y_v.shape[1])
        for s in range(y_v.shape[1]):
          val_accuracy[s]=get_accuracy(y_t_val,y_v[:,s,0])
        if using_all==True:
            loss=torch.tensor(np.ones(y_v.shape[1]))
            for j in range(y_v.shape[1]):
                loss_each = criterion(y_v[:,j,0], y_t_val)
                loss[j]=loss_each
            current_loss=loss.sum()
        else:
          current_loss=criterion(y_v[:,-1,0], y_t_val)
        if current_loss > last_loss:
            trigger_times += 1

            if trigger_times >= patience:
                print('Early stopping!')
                y_t,x_save_test = model(X_test)
                test_accuracy=np.ones(y_t.shape[1])
                for s in range(y_t.shape[1]):
                  test_accuracy[s]=get_accuracy(y_test,y_t[:,s,0])
                epoch=n_epochs+100
                # return model,accuracy,val_accuracy,test_accuracy,x_save,x_save_val,x_save_test,y_a,y_v,y_t
        else:
          trigger_times=0
        ...
        val_accuracy=None
        y_v=None
      train_loss_print=running_loss/n
      loss_ori_each[epoch]=train_loss_print
      y_t,x_save_test = model(X_test)
      test_accuracy=np.ones(y_t.shape[1])
      for s in range(y_t.shape[1]):
        test_accuracy[s]=get_accuracy(y_test,y_t[:,s,0])
      if verbose==True:
        print(f"Epoch {epoch+1}/{n_epochs}, train_loss = {train_loss_print},val_loss={current_loss},accuracy={accuracy[-1]}, val_accuracy={val_accuracy[-1]},test_accuracy={test_accuracy[-1]}")
      epoch=epoch+1
      count=count+1
  return model,accuracy,val_accuracy,test_accuracy,x_save,x_save_val,x_save_test,y_a,y_v,y_t,count,loss_ori_each
